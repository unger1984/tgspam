...
export default class SMSReg {
    static URL = "http://api.sms-reg.com/";
    ...
    async _api(method, args) {
        let opts = [{apikey: config.SMS.SMSReg.API_KEY}];
        opts = opts.concat(args)
        let url = SMSReg.URL + method + '.php?' + this._toargs(opts);
    ...
    async _waitNumber(delay) {
        ...
        while (1) {
            ...
            if (await this.getStatus()) {
                if (this.state.status === "TZ_NUM_PREPARE")
                    return true;
